...
Codex may edit `presentation.json` and add registered files below `assets/`. The desktop app watches the active Project, coalesces filesystem events, and automatically reloads a stable valid disk version in clean Edit mode. **Reload Project** remains a manual fallback. A malformed or temporarily unavailable external edit leaves the last valid presentation open; a later valid edit recovers automatically. Surviving slide and element IDs retain selection.

The app tracks the last-known disk version. Unsaved in-app edits block automatic replacement and show an external update pending; Save offers reload from disk, overwrite with the in-memory version, or cancel. Present mode, Narration Studio, and Final Video also defer application until the user returns to a clean Edit mode. No automatic JSON merge is attempted. There is no in-app AI API, chat, prompt UI, or agent execution.
